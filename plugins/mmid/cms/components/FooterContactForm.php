...
class FooterContactForm extends ComponentBase{

    public function componentDetails(){
        return [
            'name' => 'Footer Contact Form Component',
            'description' => 'Footer Contact Form Component',
        ];
    }
    ...
    public function onSubmit(){
        ...
        if(is_array($mail_receivers) && count($mail_receivers) > 0){
            foreach($mail_receivers as $mail_receiver){
                Mail::sendTo($mail_receiver['email'], 'mmid.cms::mail.footer-contact-form', $param);
            }
        }
